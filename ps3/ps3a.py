...
def get_word_score(word, n):
    ...
    """
    # TO DO...
    value_list = [SCRABBLE_LETTER_VALUES[c] for c in word]
    score = sum(value_list) * len(word)
...
def update_hand(hand, word):
    ...
    for c in hand:
        if word_dict.get(c,0) < hand[c]:
            updated_hand[c] = hand[c] - word_dict.get(c,0)
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while len(hand) > 0
        ...
        word_or_end = raw_input('Enter word, or a \".\" to indicate'
